...
socket.on('lobby_update', (players = []) => {
  ...
  if (lobbyPlayers) {
    lobbyPlayers.innerHTML = `<h3>Players in room (${players.length}):</h3><ul style="list-style:none; margin-top:8px;">` + 
      players.map(p => `<li style="padding:4px 0; color:var(--text-muted);">${p.name} ${p.id === myPlayerId ? '<strong style="color:var(--text-main);">(You)</strong>' : ''}</li>`).join('') + `</ul>`;
  }
  ...
  if (opponentsArea) {
    ...
    players.forEach(p => {
      if (p.id !== myPlayerId) {
        ...
        opp.innerHTML = `<strong>${p.name} ${!p.connected ? '🔌(Offline)' : ''}</strong><br/>🎴 ${p.handCount} cards`;
